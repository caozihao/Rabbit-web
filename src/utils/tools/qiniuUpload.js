...
const { ak, SK, bucket, privateHost, publicHost } = qiniu;
...
const getPrivateUrl = (obj) => {
  ...
  const { key, dealLine } = obj;

  let fileName = encodeURI(key);
  let downloadUrl = `${privateHost}/${fileName}?e=${dealLine}`;
  let sign = hmacSHA1(downloadUrl, SK);

  let encodedSign = Base64.stringify(sign);
  let token = `${SK}:${encodedSign}`;
